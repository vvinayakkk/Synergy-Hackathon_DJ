...
const NiftyChart = () => {
  ...
  return (
    <div className="bg-black text-white p-4 rounded-lg">
      {/* Timeframe selector */}
      <div className="flex mb-6 space-x-4 border-b border-gray-800 pb-2">
        {['1D', '5D', '1M', '6M', 'YTD', '1Y', '5Y', 'Max'].map((timeframe) => (
          <button
            key={timeframe}
            className={`px-2 py-1 text-sm ${activeTimeframe === timeframe ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-400'}`}
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data[activeTimeframe]} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
            <XAxis 
              dataKey="time" 
              axisLine={false}
              tickLine={false}
